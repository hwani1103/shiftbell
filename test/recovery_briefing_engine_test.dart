...
void main() {
  ...
  test('야간 뒤 회복 중 · 수면이 아직 부족 - 늦어도 몇 시에 잠들지와 카페인 시각을 준다', () {
    ...
    expect(b.limitations, ['최근 7일 중 수면 기록이 1일뿐이라 평균 수면은 계산하지 않았어요.']);
  });
  ...
  test('최근 7일 총 실근무시간이 48시간 이상이면 사실과 행동에 근거 EVIDENCE-012를 단다', () {
    // ⭐ 2026-09-17 재설계 - "초과근무만"에서 "기본근무+초과근무 합계"로 확장.
    // _pattern8(주간·야간 12시간씩)에서 이 시점 기준 최근 7일 기본근무만 이미 48시간
    // (12시간×4일: 야간2일+주간2일)이라 초과근무 9시간(5+4)을 더하면 총 57시간.
    final a = _analyzer(_pattern8, 5, times);
    final b = _brief(a, DateTime(2026, 9, 6, 10), [_sleep(DateTime(2026, 9, 6, 7, 30), 150)], ot: const {'2026-09-05': 300, '2026-09-03': 240});
    expect(b.facts.where((f) => f.text.startsWith('최근 7일 실근무시간 57시간')).single.evidenceIds, ['EVIDENCE-012']);
  });
