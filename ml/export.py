...
LABEL_TO_KEY = {
    ...
    "여가/휴식": "leisure",
    "기타": "etc",
    # ⭐ 2026-09-01 - 카테고리 확장(10 -> 17개). 신규 7종.
    "달리기": "running",
    "수영": "swimming",
    "등산": "hiking",
    "문화생활": "culture",
    ...
    "집안일": "housework",
    "미용": "beauty",
    # ⭐ 2026-09-03 - 카테고리 확장(17 -> 19개). 신규 2종.
    "자전거": "cycling",
    "요가/필라테스": "yoga",
    # ⭐ 2026-09-12 - 카테고리 확장(19 -> 25개). "운동"에서 분리한 신규 6종.
    # 전부 keyword_router.py 하드매핑만으로 잡히고 ML 학습 데이터는 아직
    # 없음(classes_ko에 안 나타나므로 이 매핑 자체는 지금 당장은 안 쓰이지만,
    # 나중에 이 6종 학습 데이터를 추가하면 export가 곧바로 실패하지 않도록
    # 미리 등록해둠).
    "라켓 스포츠": "racket_sports",
    "축구": "soccer",
    "농구": "basketball",
    "야구": "baseball",
    "구기종목": "ball_sports",
    "입식 격투기": "combat_sports",
}
