...
- [ ] 불규칙 모드도 배정한 날짜에 알람이 자동 생성됨 확인 (2026-09-14 출시전 감사 #26 P1 - 10일 창)
...
### 3.2 갱신 로직 (규칙적·불규칙 공통 - 2026-09-14 #26 P1)
- [ ] **하루에 한 번만 갱신** (SharedPreferences에 날짜 저장)
- [ ] **같은 날짜면 스킵**
- [ ] **다음 날이면 갱신 실행:**
  1. 근무표·템플릿·개별 예외(alarm_overrides)로 10일치(오늘~+9일) 계산 → 기존 fixed와 비교해 바뀐 것만 삭제·추가(같은 입력이면 ID·이력 불변)
  2. DB 커밋 뒤 AlarmWakeScheduler가 OS 예약 반영(실패 ID는 다음 트리거에 재시도)
  3. SharedPreferences에 갱신 날짜 저장
  4. AlarmGuardReceiver 트리거

### 3.3 AlarmRefreshReceiver 동작
- [ ] **자정 또는 날짜 변경 시 트리거됨**
- [ ] **규칙적·불규칙 모두 10일치 diff 갱신**
- [ ] **알람 이력·생성 로그는 절대 삭제하지 않음(영구 보존)** - 슬롯이 30일 지난 alarm_overrides만 정리(D11)
- [ ] **백업 복원 중에는 갱신을 미루고, 복원 완료 시 복원 작업이 직접 재조정 (2026-09-14 G4)**
...
- [ ] **DirectBootReceiver (LOCKED_BOOT_COMPLETED):**
  ...
  5. (2026-09-14 V6) 잠금 해제 후 BOOT_COMPLETED·앱 업데이트(MY_PACKAGE_REPLACED)·정확한 알람 권한 허용 때도 전체 재조정
...
- [ ] **규칙적 ↔ 불규칙 전환 뒤에도 갱신 계속(둘 다 자동 생성)**
...
- [ ] **각 이력에 아이콘 표시:**
  ...
  - deleted_by_user: 🗑️
- [ ] **"오늘", "어제", "N일 전" 표시**
- [ ] **이력은 기간과 무관하게 영구 보존됨(자동 삭제 없음)**

### 4.6 이력 삭제
- [ ] **자동 삭제 경로가 없음** (alarm_history/alarm_creation_log 영구 보존 규칙 - 2026-09-14 재확인)
...
## 카테고리 5: 백업 / 복원 (2026-09-14 출시전 감사 G4)

> 절차·합격 조건 상세: `docs/release_audit/g6/device_test_plan.md` 세션 C

- [ ] 데이터 또는 설정만 바꿔도 자동 백업이 갱신되고, 안 바꾸면 다시 쓰지 않음
- [ ] 백업 중 앱이 끊겨도 깨진 파일이 최신 백업으로 잡히지 않음
- [ ] 신규 설치 복원: 근무표·개별 예외·직접 추가한 미래 알람·일정 알림 복원, 친구공유 꺼짐
- [ ] 덮어쓰기 복원: 울리는/스누즈 중 알람이 원래 ID·원래 소리로 유지, 이력은 합쳐지고 지워지지 않음
- [ ] 잘못된 파일·근무 일정 없는 백업은 아무것도 바꾸지 않음
- [ ] 복원 중단 → 알림/시작 화면에서 이어서 복원 또는 지금 데이터로 계속
...
### 배터리 최적화
- [ ] 배터리 최적화 예외 요청 안내
- [ ] 배터리 최적화 켜져있어도 알람 울림 (기상 알람은 setAlarmClock - #20)
