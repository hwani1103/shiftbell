...
| 6 | 소유권 — `lib/providers/overtime_provider.dart` | contracts §3의 G1 의무(OT 저장 성공 뒤 `DataDomain.overtime` 통지) | §3.1 목록 밖 → G1 배정 요청. `OvertimeNotifier([Ref?])` 생성자·`adjust` 성공 뒤 revision 증가만 추가(계산 로직 무변경). release/g2·g3 변경 경로와 교집합 0건 확인 | OT 증감 저장 직후 | 미반영이면 컨디션 판정이 OT 변경을 늦게 반영(#15와 짝) | |
